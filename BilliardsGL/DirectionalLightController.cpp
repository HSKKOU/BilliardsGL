...
DirectionalLightController::DirectionalLightController(Vector3D _position, Vector3D _direction = Vector3D::down(), Vector3D _diffuse = Vector3D::one(), Vector3D _ambient = Vector3D::one(), Vector3D _specular = Vector3D::one(), Vector4D _color = Vector4D::one())
